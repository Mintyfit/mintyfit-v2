...
│   ├── shared/                       # NavbarWrapper, ShoppingCartLink, SafeHtml (ONLY sanitized-HTML site), ServiceWorkerCleanup, DeploymentCheck
...
- **Recipe mutation routes** (`/api/recipe/{update,edit,delete,regenerate-image}`) MUST call `revalidateTag('recipes')` after writes so the data cache never serves stale recipes. `regenerate-image` additionally calls `revalidatePath('/recipes')` (the catalogue is a time-based ISR page — tags don't reach it) and the client button calls `invalidateCache('recipes:')` (localStorage private-recipes list).
