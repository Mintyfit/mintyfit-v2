...
// Recipe structure call: grok-4.20 writes substantially more detailed,
// technique-rich instructions than grok-3-fast (the 2026-09 "prep patties and
// broccoli" burger was a grok-3-fast output).
const RECIPE_MODEL = 'grok-4.20-0309-non-reasoning';
...
async function callGrok(systemText, userContent, maxTokens) {
  ...
  if (!response.ok) {
    // Usage limit enforced server-side (atomic RPC in /api/grok)
    if (response.status === 429) {
      const err = await response.json().catch(() => ({}));
      if (err.error === 'LIMIT_REACHED') {
        throw new Error(`LIMIT_REACHED:${err.current ?? ''}:${err.limit ?? ''}`);
      }
    }
...
export async function generateRecipe(prompt, userId, onProgress = () => {}, { isPublic = false, isMetric = true, tier = 'free' } = {}) {
  // Usage limits are enforced server-side by /api/grok (purpose: 'recipe-generation')
  // via the atomic usage_check_and_increment RPC. A limit hit surfaces here as a
  // thrown `LIMIT_REACHED:current:limit` error, which the UI already handles.
  ...
  const recipe = await callGrok(
    `You are a professional nutritionist and chef writing recipes for home cooks. Output ONLY raw valid JSON. No markdown, no explanation, no code fences.
...
INSTRUCTION QUALITY (non-negotiable — this is what makes the recipe useful):
- Write 5–9 steps, ordered by time_marker so both components finish simultaneously.
- Each step's "instruction" is 40–80 words: the exact technique, temperatures in °C, precise timings, and visual/sensory doneness cues (color, sizzle, firmness, aroma). A beginner must be able to follow it without guessing.
- NEVER write one-line summaries like "Prep patties and broccoli" or "Season and cook". Explain HOW.
- Include a dedicated preparation step for the main component covering exact seasoning quantities and mixing/shaping technique (e.g. for burger patties: salt, pepper and spice amounts per 100g of meat, how gently to mix, patty diameter/thickness, dimpling the center, resting).
- Include oil/fat amounts, heat levels (low/medium/high + °C), and what to do while waiting (e.g. toast buns, whisk dressing).
- Steps that need them list their ingredients with amounts. Each step also has a practical "tip".${unitInstruction}`,
    `Create a complete meal recipe for: ${prompt}

Return ONLY a raw JSON object with exactly this shape (no markdown, no code fences):
{"title":"Full meal name e.g. Pan-Seared Salmon with Roasted Asparagus","main_component":"","side_component":"","description":"","meal_type":"breakfast|lunch|dinner|snack","food_type":"vegetarian|vegan|keto|paleo|pescatarian|omnivore","cuisine_type":"","price_level":"budget|medium|expensive","glycemic_load":"low|medium","cooking_technique":"baked|fried|grilled|raw|steamed|stir-fried|roasted|sautéed","calorie_range":"<300|300-600|>600","base_servings":4,"prep_time":15,"cook_time":30,"intro":"2–3 sentences: what the full meal is and why it works nutritionally","plating":"Final plating note","steps":[{"title":"","component":"main|side","time_marker":"0:00","cooking_method":"","ingredients":[{"name":"","amount":0,"unit":""}],"instruction":"40–80 words: technique, °C, timing, visual cues — no ingredient amounts","tip":""}]}`,
    6000,
  );
  ...
  let detailUrl = null;
  let thumbUrl  = null;
  const imageError2 = { upload: null };
  if (imageUrl && imageUrl.startsWith('http')) {
    const pathKey = `${Date.now()}${userId ? `-${userId.slice(0, 8)}` : ''}`;
    let uploaded = await resizeAndUploadImages(imageUrl, pathKey);
    detailUrl = uploaded.detailUrl;
    thumbUrl  = uploaded.thumbUrl;

    // One bounded server-side retry: the Ideogram URL is still live here, so a
    // transient storage failure (RLS hiccup, cold-start timeout) gets a second
    // chance before we fall back to the placeholder. Never more than one retry.
    if (!detailUrl && !thumbUrl) {
      imageError2.upload = 'first upload attempt returned no permanent URL';
      const retryKey = `${Date.now()}r${userId ? `-${userId.slice(0, 8)}` : ''}`;
      const retried = await resizeAndUploadImages(imageUrl, retryKey);
      detailUrl = retried.detailUrl;
      thumbUrl  = retried.thumbUrl;
      if (detailUrl || thumbUrl) imageError2.upload = null;
    }
  }

  // Prefer stored permanent URL; fall back to SVG placeholder.
  // NOTE: we deliberately do NOT persist the ephemeral Ideogram CDN URL — it
  // expires within hours and would leave a dead link in the DB (worse than a
  // clean placeholder). The retry above already used it while it was live.
  const image      = detailUrl || generateSvgFallback({ title: recipe.title, description: recipe.description, steps });
  const imageThumb = thumbUrl  || null;
  if (!detailUrl) {
    // Surface a helpful message when we couldn't persist a real photo
    imageError = imageError2.upload || imageError || 'Image could not be saved to permanent storage';
  }
  ...
  if (currentUser?.id) {
    ...
    if (supabaseId) {
      // Invalidate cached recipe lists so /recipes reflects the new recipe
