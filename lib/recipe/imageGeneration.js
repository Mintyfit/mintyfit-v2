...
const SAVE_IMAGE_URL     = '/api/recipe/save-image';
...
/**
 * Persist a generated image to permanent Supabase Storage via the server route
 * /api/recipe/save-image (service-role upload — bypasses the storage RLS that
 * blocked direct client inserts; migration 052 only granted SELECT).
 *
 * The server downloads the ephemeral CDN URL, resizes with sharp, uploads both
 * sizes, and returns permanent public URLs. Falls back to nulls on failure so
 * the caller can decide on a placeholder.
 *
...
export async function resizeAndUploadImages(imageUrl, pathKey) {
  try {
    const resp = await fetch(SAVE_IMAGE_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!resp.ok) {
      ...
      console.warn('resizeAndUploadImages: server save failed', resp.status, err.error || '');
      return { detailUrl: null, thumbUrl: null };
    }
    const { detailUrl = null, thumbUrl = null } = await resp.json();
    return { detailUrl, thumbUrl };
  } catch (e) {
