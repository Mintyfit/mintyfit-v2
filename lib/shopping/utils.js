...
export function extractIngredientsFromRecipe(recipe) {
  ...
  // Resolve steps from either raw DB row (instructions JSONB) or normalized recipe (steps)
  let steps = []
  if (recipe.instructions) {
    // Raw DB row: instructions is JSONB — can be legacy array or { steps: [...] }
    const instr = recipe.instructions
    const isLegacy = Array.isArray(instr)
    steps = isLegacy ? instr : (instr.steps || [])
  } else if (recipe.steps) {
    // Already-normalized recipe (has steps array)
    steps = recipe.steps
  }
  ...
  // Fallback: flat ingredients array (legacy recipes or normalized with top-level ingredients)
  if (items.length === 0 && recipe.ingredients) {
