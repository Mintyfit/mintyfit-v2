...
/**
 * pickNutritionFields — extract a clean {key: number} nutrition object from a
 * loosely-typed source (e.g. AI food-parse JSON). Only known NUTRITION_FIELDS
 * keys with finite numeric values are kept. Single canonical copy — used by
 * the assistant chat log path and JournalEntryForm.
 */
export function pickNutritionFields(source) {
  const result = {};
  if (!source || typeof source !== 'object') return result;
  for (const f of NUTRITION_FIELDS) {
    const v = Number(source[f.key]);
    if (Number.isFinite(v)) result[f.key] = v;
  }
  return result;
}
