...
- **No service worker cache at all (supersedes the two entries below)**: After THREE production incidents from SW caching (stale HTML, broken chunk streams, stale app-shell UI), the service worker cache was **removed entirely** on 2026-09-07 — product decision: the Android app shell must draw all content fresh from the web. `public/sw.js` is a self-purging no-op; `ServiceWorkerCleanup.jsx` unregisters SWs + purges caches on every load. Do NOT reintroduce a caching SW.
...
- **A WebView-wrapped site's SW cache = stale app UI**: the installed Android shell honored the site's SW caches, so after deploys it kept rendering old JS chunks (old pre-chat UI) even though the server served current code — browsers looked fine. Diagnose by asking "does the mobile BROWSER reproduce it?" — browser-OK + app-broken = shell cache. Hit 2026-09-07.
- **Android Auto Backup makes "reinstall the app" useless for staleness bugs**: with `android:allowBackup="true"` + empty backup rules, the whole WebView profile (HTTP cache, SW registrations, Cache Storage, localStorage) is backed up to the user's Google account and RESTORED on reinstall — the stale snapshot resurrects. Also: "Clear Cache" ≠ "Clear Storage" — SW registrations/Cache Storage/localStorage live in app data and survive Clear Cache. Confirmed 2026-09-10 (user cleared cache + reinstalled, still stale). Fix: shell manifest `allowBackup="false"` + `data_extraction_rules.xml` excluding all domains for cloud-backup AND device-transfer; user-side unstick = Settings → Apps → MintyFit → Storage → **Clear Storage**. Freshness self-heal code (DeploymentCheck) only protects builds that contain it — deploy it immediately after writing it.
...
- **Never self-fetch your own API route from another route handler**: `/api/recipe/regenerate-image` called `fetch(origin + '/api/ideogram')` forwarding the raw `cookie` header. Any inner auth/env/provider failure returned JSON without `data[0].url`, surfacing as the opaque "Image provider returned no URL" (2026-09-10). Fragile on three axes: public origin resolution (`new URL(request.url).origin` may be internal), forwarded-cookie staleness (outer `getUser()` can rotate the refresh token, invalidating the forwarded jar → inner 401), and Vercel deployment protection intercepting the inner call. Fix pattern: extract the core into a server lib (`lib/recipe/ideogramServer.js`, `lib/recipe/saveRecipeImageServer.js`), keep HTTP routes as thin auth wrappers for client callers, and have server routes call the lib directly. Also: always surface the provider's real status+message — never reduce to "no URL".
- **Shell-quoting/dev-server gotchas when verifying on this Windows box**: other agent projects run dev servers here (port 3000 was held by a stale iPUMPS server writing to the same temp log path). Launch dev on a dedicated port (`npm run dev -- -p 3210`), write test payload/scripts to FILES and run `powershell -NoProfile -File script.ps1` (inline `Set-Content; curl` chains with mixed quotes hang silently), and kill stray node only by `StartTime` filter.